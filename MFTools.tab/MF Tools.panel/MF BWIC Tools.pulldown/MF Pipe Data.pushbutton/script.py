...
# VERTICAL BWIC
BWICFamilySymbol = doc.GetElement(ElementId(4385149))
...
for pipe in pipes:
	...
	for level in levels:
		...
		if pipeTop > levelZ and pipeBottom < levelZ:
			...
			insulation_thickness = pipe.get_Parameter(BuiltInParameter.RBS_REFERENCE_INSULATION_THICKNESS).AsDouble() # ft
			...
			outside_diameter = pipe.get_Parameter(BuiltInParameter.RBS_PIPE_OUTER_DIAMETER).AsDouble()  #ft
			...
			rounded_hole = roundup(( min_hole_size)*304.8, 50) / 304.8  # add margin and round up to nearest 50mm
			...
			b.LookupParameter("MF_Length").Set(BWIC_height)
			b.LookupParameter("MF_Depth").Set(100/304.8)
