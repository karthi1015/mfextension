# -*- coding: utf-8 -*-
__title__ = 'Reload Families from Project Folder'
__doc__ = """Reload Families from Project Folder
"""
...
loadPath = basePath + "\Project Families\@_From Template"

print loadPath
#sys.exit()
output = []
output.append(["Family Name", "FilePath"])
t = Transaction(doc, 'Reload Families')
t.Start()

for family in families:
	try:
		
		reloadFamilyOption = IFamilyLoadOptions  ##error: expected IFamilyLoadOptions, got type
		
		fName = family.Name
		famPath = loadPath + "\\" + fName + ".rfa"
		...
		reloadedFamily = doc.LoadFamily(famPath, reloadFamilyOption, family)
