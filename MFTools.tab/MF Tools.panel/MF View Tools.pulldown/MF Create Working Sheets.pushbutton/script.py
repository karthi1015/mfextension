# -*- coding: utf-8 -*-
__title__ = 'MF Create Working Sheets'
__doc__ = """MF Create Working Sheets
"""
...
for level in levels:		
	...
	for fvt in foregroundViewTemplates:
		try:
			...
			fv.Name = fvt.Name + " - " + level.Name + " - Foreground"
	...
	#for bvt in backgroundViewTemplates:
		...
		new_sheet.Name = "Working Sheet - " + 	fvt.Name + " - " + level.Name	
		
		## create sensible sheet number here... 
		
		vp_location = template_vp_location
		
		#add background view (bv) to sheet
		bvp = Viewport.Create(doc, new_sheet.Id, bv.Id,  vp_location )
		#add forgreound view (fv) to sheet
